...
final class Providers<T> {
  ...
  TypeLiteral<ListenableFuture<?>> futureTypeLiteralFrom(TypeToken<?> type) {
    return (TypeLiteral<ListenableFuture<?>>) TypeLiteral.get(
        (LISTENABLE_FUTURE.isSupertypeOf(type)
  ...
  private final class EventualProvider<V>
      implements
        ProviderWithDependencies<ListenableFuture<V>> {
    ...
    public ListenableFuture<V> get() {
      ...
      return Futures.catchingAsync(outputResult, Throwable.class, catchingFunction, executor);
    }

    private final AsyncFunction<Throwable, V> catchingFunction = new AsyncFunction<Throwable, V>() {
      @Override
      public ListenableFuture<V> apply(@Nullable Throwable t) throws Exception {
